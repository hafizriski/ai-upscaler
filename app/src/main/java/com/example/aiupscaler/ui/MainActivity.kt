...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.toggleBackend.check(binding.btnGpu.id)
        binding.toggleBackend.addOnButtonCheckedListener { _, checkedId, isChecked ->
            if (isChecked) {
                backend = if (checkedId == binding.btnGpu.id) Backend.GPU else Backend.CPU
            }
        }

        binding.btnPick.setOnClickListener { pick.launch("image/*") }
        binding.btnUpscale.setOnClickListener {
            sourceBitmap?.let { runUpscale(it) }
                ?: toast(getString(com.example.aiupscaler.R.string.error_pick))
        }
    ...
    private fun runUpscale(src: Bitmap) {
        ...
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                val engine = UpscalerInterpreter(
                    this@MainActivity,
                    "models/realesr_general_x4v3.tflite",
                    backend
                )
                val out = try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    binding.tvInfo.text = getString(
                        ...
                        com.example.aiupscaler.R.string.backend_label, backend.name
                    )
                }
            } catch (e: Throwable) {
                withContext(Dispatchers.Main) {
                    ...
                    Snackbar.make(
                        binding.root,
                        getString(com.example.aiupscaler.R.string.error_process, e.message ?: ""),
    ...
    private fun save() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val name = "upscaled_${System.currentTimeMillis()}.png"
            ...
            uri?.let {
                ...
                withContext(Dispatchers.Main) {
                    toast(getString(com.example.aiupscaler.R.string.saved_ok))
                }
            }
        }
    }
